/*********************************************************************
** Program Filename: Assignment 4 - dynArray.c
** Author: David Mednikov
** Date: 11/12/17
** Description: Implement a heap using a dynamic array.
*********************************************************************/
...
TYPE getMinHeap(DynArr *heap)
{
	...
	assert(heap != NULL); /* heap is not null */
	assert(heap->size > 0); /* heap is not empty */
	
  	temp = getDynArr(heap,0); /* get first element of heap */
...
void addHeap(DynArr *heap, TYPE node)
{
	...
	addDynArr(heap, node); /* add value to end of array */
	
	int pos = heap->size - 1; /* get index of newly added value */
	int parent;
	
	while (pos != 0) /* while the current element is not the root */
	{
		parent = (pos - 1)/2; /* get index of parent */
		
		if (compare(getDynArr(heap, pos), getDynArr(heap, parent)) == -1) /* current element is less than parent */
		{
			swapDynArr(heap, pos, parent); /* swap values */
			pos = parent;
		}
...
void _adjustHeap(DynArr *heap, int max, int pos)
{
	...
	int leftChild = 2 * pos + 1; /* left and right children */
	int rightChild = 2 * pos + 2;
	
	if (rightChild < max) /* there are two children */
	{
		smallestChildIndex = _smallerIndexHeap(heap, leftChild, rightChild); /* get the smaller of the two children */
			
			if (compare(getDynArr(heap, smallestChildIndex), getDynArr(heap, pos)) == -1) /* if value at smaller child is less than current position, they need to be swapped */
			{
				swapDynArr(heap, pos, smallestChildIndex); /* swap values */
				_adjustHeap(heap, max, smallestChildIndex); /* restore heap */
			}
	}
	
	else if (leftChild < max) /* there is only one child */
	{		
		if (compare(getDynArr(heap, leftChild), getDynArr(heap, pos)) == -1) /* if value at child is less than current position, they need to be swapped */
		{
			swapDynArr(heap, pos, leftChild); /* swap values */
			_adjustHeap(heap, max, leftChild); /* restore heap */
		}
	}
	
	else /* no children, so the heap is adjusted */
		return;
}
...
{
  	assert(heap != NULL); /* heap is not null */
	assert(heap->size > 0); /* elements can be deleted */
	
	int last = heap->size - 1; /* get index of last element */
	
	heap->data[0] = heap->data[last]; /* copy last element to root */
	
	removeAtDynArr(heap, last); /* remove last element */
	_adjustHeap(heap, last, 0); /* percolate "root" to its proper place and restore the heap */
...
void _buildHeap(DynArr *heap)
{
	...
	int max = heap->size; /* get size of heap */
	
	int i;
	
	for (i = (max/2) - 1; i >= 0; i--) /* adjust the heap from the end leaf to the root */
...
void sortHeap(DynArr *heap)
{
	...
	_buildHeap(heap); /* create heap from array */
	
	int i;
	
	for (i = heap->size - 1; i > 0; i--) /* swap root with last element, and work way towards the root */
	{
		swapDynArr(heap, 0, i);
