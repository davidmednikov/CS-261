...
int dfsIterative(Graph* graph, Vertex* source, Vertex* destination)
{
	...
	while (stack->size > 0)
	{
		Vertex* pop = dequeBack(stack);
		dequePopBack(stack);

		if (pop->label == destination->label)
			return 1;
		
		if (pop->isVisited == 0)
		{
			dequePushBack(reachable, pop);
			pop->isVisited = 1;
		}

		Vertex** neighbors = pop->neighbors;
		Vertex* current = NULL;

		for (int i = 0; i < pop->numNeighbors; i++)
		{
			current = neighbors[i];

			if (current->isVisited == 0)
				dequePushBack(stack, current);
